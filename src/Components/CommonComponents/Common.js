...
export const validateConsecutiveSequence = num => {
  ...
  for (let i = 0; i <= loopIndex; i++) {
    if (
      parseInt(numArray[i]) + 1 === parseInt(numArray[i + 1]) &&
      parseInt(numArray[i + 1]) + 1 === parseInt(numArray[i + 2])
    ) {
      return true;
    } else if (
      parseInt(numArray[i]) - 1 === parseInt(numArray[i + 1]) &&
...
export const validatePinLength = num => {
  // Breaking the pin into array
  var numArray = num.toString().split("");

  // Validate precedding zeros and total legth
  if (numArray.length === 3 && numArray[0] !== 0) {
    numArray.unshift("0");
...
export const prepareExcludeNumbers = () => {
  ...
  for (let num = config.minPinValue; num <= config.maxPinValue; num++) {
    // Validate PIN Length
    var pinCheck = validatePinLength(num);
